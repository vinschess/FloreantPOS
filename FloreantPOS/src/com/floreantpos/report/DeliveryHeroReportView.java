...
public class DeliveryHeroReportView extends JPanel {
	private JXDatePicker fromDatePicker = UiUtil.getCurrentMonthStart();
	private JXDatePicker toDatePicker = UiUtil.getCurrentMonthEnd();
	private JButton btnGo = new JButton(com.floreantpos.POSConstants.GO);
	private JPanel reportContainer;
	
	public DeliveryHeroReportView() {
		...
		btnGo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					POSMessageDialog.showError(DeliveryHeroReportView.this, POSConstants.ERROR_MESSAGE, e1);
				}
			}
	...
	private void viewReport() throws Exception {
		...
		List<DeliveryHeroTransaction> transactions = (List<DeliveryHeroTransaction>) PosTransactionDAO.getInstance().findTransactions(null, DeliveryHeroTransaction.class, fromDate, toDate);
		
		int saleCount = 0;
		double totalSales = 0;
		double totalTips = 0;
		for (DeliveryHeroTransaction transaction : transactions) {
			++saleCount;
		...
		map.put("reportTitle", "========= DELIVERY HERO REPORT ==========");
		...
		JasperReport jasperReport = ReportUtil.getReport("delivery-hero-report");
		JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, map, new JRTableModelDataSource(new DeliveryHeroReportModel(transactions)));
	...
	public static void main(String[] args) {
		...
		PanelTester.test(new DeliveryHeroReportView());
	}
}
