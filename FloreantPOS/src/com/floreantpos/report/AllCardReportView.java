...
public class AllCardReportView extends JPanel {
	private JXDatePicker fromDatePicker = UiUtil.getCurrentMonthStart();
	private JXDatePicker toDatePicker = UiUtil.getCurrentMonthEnd();
	private JButton btnGo = new JButton(com.floreantpos.POSConstants.GO);
	private JPanel reportContainer;
	
	public AllCardReportView() {
		...
		btnGo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					POSMessageDialog.showError(AllCardReportView.this, POSConstants.ERROR_MESSAGE, e1);
				}
			}
	...
	private void viewReport() throws Exception {
		...
		List<AllCardTransaction> transactions = (List<AllCardTransaction>) PosTransactionDAO.getInstance().findTransactions(null, AllCardTransaction.class, fromDate, toDate);
		
		int saleCount = 0;
		double totalSales = 0;
		double totalTips = 0;
		for (AllCardTransaction transaction : transactions) {
			++saleCount;
		...
		map.put("reportTitle", "========= All CARD REPORT ==========");
		...
		JasperReport jasperReport = ReportUtil.getReport("all-card-report");
		JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, map, new JRTableModelDataSource(new AllCardReportModel(transactions)));
	...
	public static void main(String[] args) {
		...
		PanelTester.test(new AllCardReportView());
	}
}
