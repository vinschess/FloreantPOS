...
public class MenuLogReportView extends JPanel {
	private JXDatePicker fromDatePicker = UiUtil.getCurrentMonthStart();
	private JXDatePicker toDatePicker = UiUtil.getCurrentMonthEnd();
	private JButton btnGo = new JButton(com.floreantpos.POSConstants.GO);
	private JPanel reportContainer;
	
	public MenuLogReportView() {
		...
		btnGo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					POSMessageDialog.showError(MenuLogReportView.this, POSConstants.ERROR_MESSAGE, e1);
				}
			}
	...
	private void viewReport() throws Exception {
		...
		List<MenuLogTransaction> transactions = (List<MenuLogTransaction>) PosTransactionDAO.getInstance().findTransactions(null, MenuLogTransaction.class, fromDate, toDate);
		
		int saleCount = 0;
		double totalSales = 0;
		double totalTips = 0;
		for (MenuLogTransaction transaction : transactions) {
			++saleCount;
		...
		map.put("reportTitle", "========= MENULOG REPORT ==========");
		...
		JasperReport jasperReport = ReportUtil.getReport("menu-log-report");
		JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, map, new JRTableModelDataSource(new MenuLogReportModel(transactions)));
	...
	public static void main(String[] args) {
		...
		PanelTester.test(new MenuLogReportView());
	}
}
