...
public class EatNowReportView extends JPanel {
	private JXDatePicker fromDatePicker = UiUtil.getCurrentMonthStart();
	private JXDatePicker toDatePicker = UiUtil.getCurrentMonthEnd();
	private JButton btnGo = new JButton(com.floreantpos.POSConstants.GO);
	private JPanel reportContainer;
	
	public EatNowReportView() {
		...
		btnGo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					POSMessageDialog.showError(EatNowReportView.this, POSConstants.ERROR_MESSAGE, e1);
				}
			}
	...
	private void viewReport() throws Exception {
		...
		List<EatNowTransaction> transactions = (List<EatNowTransaction>) PosTransactionDAO.getInstance().findTransactions(null, EatNowTransaction.class, fromDate, toDate);
		
		int saleCount = 0;
		double totalSales = 0;
		double totalTips = 0;
		for (EatNowTransaction transaction : transactions) {
			++saleCount;
		...
		map.put("reportTitle", "========= EAT NOW REPORT ==========");
		...
		JasperReport jasperReport = ReportUtil.getReport("eat-now-report");
		JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, map, new JRTableModelDataSource(new EatNowReportModel(transactions)));
	...
	public static void main(String[] args) {
		...
		PanelTester.test(new EatNowReportView());
	}
}
